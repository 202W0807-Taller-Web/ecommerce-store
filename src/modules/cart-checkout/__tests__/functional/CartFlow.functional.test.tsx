...
describe("CartPage - Test Funcional del Carrito", () => {
  ...
  it("debe deshabilitar el botón + cuando se alcanza el stock máximo", () => {
    ...
    mockGetProductStockStatus.mockImplementation(
      (idProducto: number, cantidadSolicitada: number) => {
        if (idProducto === 1) {
          return {
            idProducto,
            stockDisponible: 2,
  ...
  it("debe deshabilitar el botón de continuar cuando el carrito es inválido", () => {
    ...
    const continueButton = screen.getByRole("button", {
      name: /Iniciar sesión para continuar/i,
    });
    expect(continueButton).toBeDisabled();

    expect(screen.getByText(/Problemas con el stock/i)).toBeInTheDocument();
    expect(
      screen.getByText(/Algunos productos ya no tienen stock disponible/i),
    ).toBeInTheDocument();
  });
